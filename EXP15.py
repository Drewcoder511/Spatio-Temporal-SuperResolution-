...
Comparison of EXP12 (Without Node Embedding)

Encoder: SageGCN: 1 to 6 (Without center node feature because of full connected graph)
Decoder: Decoder: Transformer: 6 to 20 to 1 xxxxxxxxxxx
...
for epoch in range(10):
    ...
    for i in range(100,10000,100):
        ...
        for batch in range(100):  # each time frame
            ...
            for index, j in enumerate(sorted(time_node)):
                Input = torch.unsqueeze(features_tensor[i+batch+j-20,time_node[j]], 1)
    ...
    for i in range(10000, 17000):
        ...
        src = torch.zeros(1, len(time_node), 6)
        for index, j in enumerate(sorted(time_node)):
            Input = torch.unsqueeze(features_tensor[i+j-20,time_node[j]], 1)
            output = Encoder(Input)
            src[0, index, :] = output
    ...
    if Loss < best_valid_loss:
        ...
        print("save!")
    torch.save(valid_pred, "/Users/huoshengming/Downloads/Best_model/EXP15_best_pred" + str(epoch))
